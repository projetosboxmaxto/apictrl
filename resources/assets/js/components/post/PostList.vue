...
<table id="table_data" class="table table-bordered table-striped display" style="width: 100%">
  ...
  <post v-if="action =='form'" v-bind:id_load="id" 
          v-bind:onSave="onSave" v-bind:post_type="post_type"
...
<script>
    export default {
        ...
        mounted() {
           ...
           self.button_new_text =  "<i class=\"fa fa-file\" ></i> NOVA NOTÍCIA";
           
           if ( self.type == "page"){
              self.button_new_text = "<i class=\"fa fa-html5\" ></i> NOVA";
              self.show_new_button = false;
           }

           if ( self.type == "news"){
              self.button_new_text = "<i class=\"fa fa-envelope-o\" ></i> NOVA NEWSLETTER";
           }
